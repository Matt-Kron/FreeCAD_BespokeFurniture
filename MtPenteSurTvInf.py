...
PARAM_GROUP = "User parameter:BaseApp/Preferences/Macros/MtPenteSurTvInf"
...
class AssignmentDialog(QtWidgets.QDialog):
    ...
    def setupUi(self, all_data, pref_show):
        ...
        self.edits = {}
        for i, key in enumerate(["montant", "role1"]):
            ...
            if self.assigned[key]:
                # print(f"key {key}, self.assigned[key] {self.assigned[key]}")
...
def apply_assignments(m_name, r1_name, p_type):
    try:
...
def run_assignment_macro(force_ui=None):
    ...
    if len(resolved_parts) == 2:
        for p in resolved_parts:
            ...
            if found:
                ...
                r1_name = [o for o in resolved_parts if o.Name != m_name][0].Name
                break
    else:
        show_ui = True # On n'a pas 3 pièces distinctes -> Interface obligatoire

    if not show_ui and m_name and r1_name:
        if apply_assignments(m_name, r1_name, m_type):
    ...
    if dlg.exec_() == QtWidgets.QDialog.Accepted:
        ...
        res = dlg.assigned
        apply_assignments(res["montant"], res["role1"], dlg.prop_type)
